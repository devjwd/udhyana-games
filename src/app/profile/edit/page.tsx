...
export default function EditProfilePage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await updateUserProfile(session?.user?.id as string, { fullName, phone, image });
      ...
      <main className={styles.main} style={{ justifyContent: 'center', alignItems: 'flex-start', minHeight: '80vh', padding: '2rem' }}>
        ...
        <div style={{ maxWidth: '800px', width: '100%', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <form onSubmit={handleUpdateProfile} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                  <img src={image || 'https://via.placeholder.com/150'} alt="Avatar Preview" style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--primary-accent)' }} />
          ...
          <div className={styles.profileCard}>
            ...
            {passwordMsg.text && (
              <div style={{ background: passwordMsg.type === 'error' ? 'rgba(255,77,77,0.1)' : 'rgba(193,255,28,0.1)', border: `1px solid ${passwordMsg.type === 'error' ? 'rgba(255,77,77,0.3)' : 'rgba(193,255,28,0.3)'}`, borderRadius: '6px', padding: '0.85rem 1rem', marginBottom: '1.5rem', color: passwordMsg.type === 'error' ? '#ff6b6b' : 'var(--primary-accent)' }}>
                {passwordMsg.text}
              </div>
            )}
